create table if not exists public.system_issue_reports (
  ...
  category text not null check (category in ('LIBRARY','WORKSPACE','DISCOVERY','AI_CHATBOT','OTHER')),
  ...
  page_path text, attachments jsonb not null default '[]'::jsonb, status text not null default 'OPEN' check (status in ('OPEN','IN_PROGRESS','RESOLVED','DISMISSED')),
  admin_note text,
